...
def FashionMNIST(train=True, batch_size=None, augm_flag=True):
    if batch_size == None:
        if train:
    ...
    transform_test = transforms.Compose([transforms.Pad(1,fill=0),transform_base])

    transform_train = transforms.RandomChoice([transform_train, transform_train])
    transform = transform_train if (augm_flag and train) else transform_train
